...
<div class="header">
   ...
   <div class="living_middle">
		...
		<ul class="nav nav-tabs"><li  ><a data-toggle="tab" href="manager.php">Checkout</a></li>
						...
						<li class="active"><a data-toggle="modal" href="post.php" >Blog Post</a></li>
...
$title = formPrepare($_POST['title']);
$body = formPrepare($_POST['body']);
$date = date('D-M-Y');
		  
		  
	  
  $target_dir = "blogphotos/"; 
  $target_file = $target_dir . basename($_FILES["img"]["name"]);

     (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)); 

	 
	 $sqli= "INSERT INTO blog (Title, Body, Photo, Poster, Date) VALUES('$title', '$body', '$target_file', '$userId', '$date')";
$query = mysqli_query($conn, $sqli); 

if($query==TRUE){
	$insert ="<div class='alert alert-success  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Your post was successfully uploaded. </div>";
 
	
	
}else{$inserter ="<div class='alert alert-warning  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Sorry, Your post was not published. Please try again </div>";
 }
...
<textarea  name="body" style="min-height:300px" class="form-control" /></textarea>
<br />
 <p>Upload Image  Thumbnail</p>
   <input type="file" name="img" required class="form-control" />
   
	<input type="submit" name="post" value="Post Story" class="btn btn-success" />
