...
$room_Number=$room_Numerr=$price=$priceer=$description=$descriptionerr=$img=$imger=$fileer=$insert=$inserter="";

  
  if(isset($_POST['add'])){
	  if(empty($_POST['num'])){$room_Numerr= "<div class='alert alert-warning  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Room number cannot be empty </div>";
   
		  }else{$room_Number= formPrepare($_POST['num']);}
...
if(empty($_POST['price'])){ $priceer = "<div class='alert alert-warning  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Please input a valid price </div>";
		  ...
		  if(empty($_POST['description'])){$descriptionerr= "<div class='alert alert-warning  alert-dismissable fadein'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Please write at least a little description about the room </div>";
   
		  }else{$description= formPrepare($_POST['description']);}
  ...
  $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
	 ...
	 $sqli= "INSERT INTO rooms (Number,Price, Description, Photo) VALUES('$room_Number', '$price', '$description', '$target_file')";
$query = mysqli_query($conn, $sqli); 

if($query==TRUE){
	$insert ="<div class='alert alert-success  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>You have successfully added a room. </div>";
 
	
	
}else{$inserter ="<div class='alert alert-warning  alert-dismissable'><a href='#' class='close' data-dismiss='alert' arial-label='close'>&times;</a>Sorry, the room number has already been registered. Please try another one. </div>";
 }
...
<div class="header">
   ...
   <div class="living_middle">
		...
		<ul class="nav nav-tabs"><li  ><a data-toggle="tab" href="manager.php">Checkout</a></li>
						...
						<li class="active"><a data-toggle="modal" href="regroom.php" >Register Room</a></li>
		 ...
		 <!-- The form for registring a room is here -->
		 <?php echo $insert; echo $inserter; ?>
		 <form action="" method="post"  enctype="multipart/form-data">		 
		  <h3>Register A Room</h3>	
		  
		 <p>Enter Room number</p>
		 <input type="text" name="num"  value="<?php echo $room_Number; ?>"  class="form-control"  />		 	
		   <?php echo $room_Numerr; ?>
		 
		 <p>Enter Price of room</p>
		 <input type="text" name="price" value=""  class="form-control"  />
		  <?php echo $priceer; ?>
		 <br />
		 
		  <p>Enter brief description of the room</p>

<textarea name="description" value="<?php echo $description; ?>"  class="form-control" /></textarea>
 <?php echo $descriptionerr; ?>
 <p>Upload Image discription of the room</p>
   
   <input type="file" name="fileToUpload" class="form-control" />
   
    <?php echo $fileer; ?>
		 <input type="submit" name="add" value="Add a Room" class="btn btn-success" />
